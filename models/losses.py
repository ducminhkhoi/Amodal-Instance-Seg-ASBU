...
class MaskWeightedNLLMumfordShah(nn.Module):
    ...
    def forward(self, predict, target, mask):
        ...
        norm_loss = kornia.filters.spatial_gradient(mean.unsqueeze(1)).norm(p=1, dim=2)[:, 0]

        pre_loss = F.binary_cross_entropy(mean, target, reduction='none') + norm_loss * 0.1
...
class MaskWeightedNLLMumfordShahBoundary(nn.Module):
    ...
    def forward(self, predict, target, mask):
        ...
        pre_loss = F.binary_cross_entropy(mean, target, reduction='none')

        pre_loss = (self.inmask_weight * pre_loss[mask==1].sum() + self.outmask_weight * pre_loss[mask==0].sum()) 
        
        std = kornia.filters.spatial_gradient(mean.unsqueeze(1)).norm(p=1, dim=2)[:, 0]

        norm_loss = F.mse_loss(std, std_target, reduction='none')

        norm_loss = (self.inmask_weight * norm_loss[std_target==1].sum() + self.outmask_weight * norm_loss[std_target==0].sum())
        
        loss = (pre_loss + norm_loss) / (n * h * w)
